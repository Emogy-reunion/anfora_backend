...
def generate_invoive():
  #kuchukua data from the frontend
  ...
  #Single Supplement Rate and the km 
  src_rate=float(data.get(src_rate),0)
  psv_rate=float(data.get(psv_rate))
  
  #Hesabu kidogo tu 
  accomadation=num_people * overnight_cost* total_days
  total_src=src_count*src_rate
  raw_transport= psv_km*psv_km
  pf_markup=raw_transport*1.10
  driver_cost=vehicle_driver*total_days
  
  total_cost=total_src + pf_markup + driver_cost + accomadation
  ...
  return jsonify({
        ...
        "invoice_metadata": {
            ...
            "itinerary_breakdown": itinerary_days
        },
        "breakdown": {
            "accommodation": round(total_accommodation, 2),
            "single_supplement_srs": round(total_srs, 2),
            "transport_pf_km_10_percent": round(total_pf_km_markup, 2),
            "vehicle_driver": round(total_vehicle_driver, 2)
        },
        "totals": {
